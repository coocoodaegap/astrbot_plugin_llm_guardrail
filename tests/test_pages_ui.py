...
class GuardrailPagesUiTests(unittest.TestCase):
    def test_pages_use_documented_tabs_and_visual_rule_editor(self):
        ...
        self.assertIn('apiGet("get_shared_constants")', javascript)
        ...
        self.assertIn("void loadTabData(name);", javascript)
        self.assertIn("async function loadTabData(name)", javascript)
        self.assertIn("function applyRuleLibraryPayload", javascript)
        self.assertIn("function applyPolicyLibraryPayload", javascript)
        self.assertIn("function applySystemSettingsPayload", javascript)
        self.assertNotIn("已加载规则库 revision", javascript)
        self.assertNotIn("已加载系统设置 revision", javascript)
        self.assertIn('publishReport(payload?.error || "无法读取访问控制状态。"', javascript)
        self.assertIn('publishReport(payload?.error || "无法读取会话策略状态。"', javascript)
        self.assertIn('publishReport(payload?.error || "无法读取 RAG 经验记录。"', javascript)
        self.assertIn("检测到未保存的规则草稿", javascript)
        self.assertIn("检测到未保存的策略草稿", javascript)
        self.assertIn("检测到未保存的系统设置草稿", javascript)
        self.assertNotIn("async function refresh()", javascript)
        self.assertIn("function renderRuleList", javascript)
        self.assertIn("function hitActionsForRail", javascript)
        ...
        self.assertIn('hitActionsForRail(node.rail)', javascript)
        self.assertIn('outputDefaultHitActionField[3].unshift("observe")', javascript)
        ...
        self.assertNotIn('"strengthen_prompt"', rule_template_block)
        self.assertIn("strengthen_prompt: {", javascript)
        self.assertIn('rails: new Set(["prompt_rail"])', javascript)
        ...
        self.assertIn("pendingRuleDeletionIds = new Set()", javascript)
        self.assertIn("is-pending-deletion", javascript)
        self.assertIn("待删除 · 点击取消", javascript)
        self.assertIn("rule-pending-deletion-marker", (PAGES_DIR / "style.css").read_text(encoding="utf-8"))
        ...
        self.assertNotIn('id="policy-graph-editor-status"', html)
        self.assertIn("function setPolicyGraphEditorStatus", javascript)
        self.assertIn('publishReport(message, { tone: isError ? "error" : "success" })', javascript)
        ...
        self.assertIn("function allocatePolicyBindingId", javascript)
        self.assertIn("function renamePolicyNode", javascript)
        self.assertIn("rewritePolicyDependencyNodeId", javascript)
        self.assertIn("rewritePolicyLogicInputNodeId", javascript)
        self.assertIn("rewritePolicyTemplateNodeIds", javascript)
        ...
        self.assertIn("function clearPolicyGraphNodeDependency", javascript)
        self.assertIn("function handlePolicyGraphShortcut", javascript)
        self.assertIn('document.addEventListener("keydown", handlePolicyGraphShortcut)', javascript)
        self.assertIn("depend_on）· D", javascript)
        self.assertIn("depend_on）· ⇧D", javascript)
        self.assertIn("从当前策略移除 · ⇧Del", javascript)
        ...
        self.assertIn("experience_candidate_threshold", javascript)
        self.assertIn('min: 0, max: 1, step: "0.01"', javascript)
        self.assertIn("field.min !== undefined", javascript)
