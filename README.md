...
> 当前为 **v0.7.1 测试版**。欢迎用于真实群聊或私聊环境，但建议先从观察模式和少量策略开始配置。
...
| Pages 中的名称 | 内部字段名 | 含义 |
| --- | --- | --- |
| Binding ID | `binding_id` | 节点在当前策略图中的可修改身份；规则节点的来源 `rule_id` 不随它改变。旧元件的 `component_id` 仅作为兼容镜像保留。 |
...
| 知识库经验 | 查看、编辑或删除 RAG 经验候选记录；默认记录命中，`rag_judge` 的经验候选阈值可额外保留高分未命中结果。 |
