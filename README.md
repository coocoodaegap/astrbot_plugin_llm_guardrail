# astrbot_plugin_llm_guardrail

面向 AstrBot 的 LLM Guardrail 编排插件，用于在一次 LLM 调用前后执行输入检查、请求路由、最终请求检查、提示词加固和输出检查。

## 已实现功能

### 输入检查

- 在 `on_waiting_llm_request` 执行，优先检查触发本次 LLM 请求的用户输入。
- 支持关键词、正则和逻辑门规则。
- 支持观察、拦截和净化输入。
- 输入被拦截时可返回占位提示，也可尽量静默阻断。
- 输入净化会写回消息事件文本，后续插件和 AstrBot 主流程会看到净化后的内容。
- 规则结果会进入本轮上下文，可供后续路由、最终请求检查、提示词加固和输出检查通过 `depend_on` 引用。

### 请求模型路由

- 在 `on_waiting_llm_request` 执行，晚于输入检查且早于 AstrBot 构造 LLM 请求。
- 支持根据前置规则结果切换当前 UMO 的聊天 Provider。
- 支持 first-hit 路由策略。
- 响应阶段会尝试恢复切换前的 Provider。

### 最终请求检查

- 在 `on_llm_request` 阶段执行，早于提示词加固。
- 默认关闭，开启后检查当前 `ProviderRequest.prompt`。
- 支持关键词、正则和逻辑门规则。
- 支持观察、拦截和净化最终请求文本。
- 规则结果会进入本轮上下文，可与先前阶段结果一起供后续规则通过 `depend_on` 引用。

### 提示词加固

- 在 `on_llm_request` 阶段执行，晚于最终请求检查。
- 支持整体替换用户 prompt。
- 支持向 system prompt 前缀、system prompt 后缀、临时用户上下文或输入包装中注入加固文本。
- 支持通过前置规则结果有条件地执行加固。

### 输出检查

- 在 `on_llm_response` 阶段执行。
- 支持检查 `LLMResponse.completion_text`。
- 支持关键词、正则和逻辑门规则。
- 支持观察、拦截和净化输出。
- Step 5 可使用 `retry_generation`：命中后以本轮文本请求快照直接调用本轮已选中的 Provider 重新生成，并对新文本重新执行完整 Step 5；旧的 Step 5 结果会被丢弃。首版仅支持非流式纯文本请求，Provider 不可用、超时、失败或次数耗尽都会一次性阻断，且不会回退到备用 Provider。仅当最终文本通过 Step 5 后才会提交给 AstrBot 写入会话历史；最终阻断不会写入候选回复。
- 流式 chunk 响应会被跳过。

### RAG 知识经验

- `rag_judge` 命中时会保存一条本地经验记录，包含实际检查文本及最高分检索证据的知识库、文档和分数来源。
- Pages 的“知识库经验”页支持筛选、查看、编辑和删除本地记录，并通过 revision 防止旧页面覆盖新编辑。
- 编辑保存后，可把内容作为新的 Markdown 文档写入该次最高分证据所属的原知识库；来源不明确时不会猜测写入目标。
- Guardrail 不跟踪、不修改也不删除已成功写入 AstrBot 知识库的文档；删除经验记录也不会影响该文档。

### 配置与诊断

- 支持按群聊/私聊分别设置会话进入 Guardrail、跳过放行或跳过拦截。
- 支持管理员使用 `/guardrail` 查看当前插件状态、当前 UMO、各 rail 启用状态和规则数量。
- 支持 debug 日志输出本轮命中规则、路由结果、提示词变更摘要和 warning 摘要。

## 当前链路

```text
on_waiting_llm_request(priority=1_000_000):
  访问控制

on_waiting_llm_request(priority=-1_000_000):
  Step 1 输入检查
  Step 2 请求模型路由

on_llm_request(priority=-1_000_000):
  Step 3 最终请求检查
  Step 4 提示词加固

on_llm_response(priority=1_000_000):
  Step 5 输出检查
```

## 兼容性

当前插件框架要求为 AstrBot `>=4.26.0,<5`。
