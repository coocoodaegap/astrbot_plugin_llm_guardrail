...
> 当前为 **v0.4.0 测试版**。欢迎用于真实群聊或私聊环境，但建议先从观察模式和少量策略开始配置。
...
| P3 数据流与并发 | 同一策略执行内可显式消费受限 payload、使用输出重定向；同一 Rail 的已就绪检查可并发执行，并按稳定顺序结算。 |
| 策略包迁移 | 支持规则包与自包含策略包的导出、预览导入、ID 冲突处理和 revision 原子提交。 |
...
- 终止性 `block` 优先于输出重定向和 `retry_generation`。P4 路线图中的多轮升级判断、备用 Provider、流式检测、通用 payload schema 和自定义脚本运行时均未默认启用。
...
P0 至 P3 已完成。下一阶段会在独立设计和匿名观察后，再评估多轮风险信号、深层提示词泄露判断、扩展编码/协议、备用 Provider、流式输出检查和通用策略数据面。试用期间不会通过隐藏开关或 system fallback 自动启用这些能力。
