...
- `compose_text` 现在可放置在全部五个 Step；其说明统一为面向当前或后续阶段的显式模板消费者，不再将用途特指为 Step 4 提示词增强。
- `context_extractor` 现在同样可放置在全部五个 Step，并继续作为只读、固定观察的运行期上下文数据元件。
- 规则创建及策略规则／元件选择弹窗调整标题层级、Binding ID 后间距和右上角操作区；元件选择弹窗扩大为比规则选择弹窗更宽的三列友好布局，并按元件 key 的字典序显示选项。
